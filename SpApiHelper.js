...
function makeSpApiRequest(url, method, body, accessToken, options) {
  options = options || {};
...
function makeSpApiRequestWithRetry(url, method, body, accessToken, options) {
  options = options || {};
  const maxRetries = options.maxRetries || 3;
  const retryDelay = options.retryDelay || 5000;

  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    const response = makeSpApiRequest(url, method, body, accessToken, options);
...
function handleSpApiError(response, operationName) {
...
function buildSpApiUrl(endpoint, path, queryParams) {
