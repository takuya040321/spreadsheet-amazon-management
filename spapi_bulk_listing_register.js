...
// ============================================
// ASINから商品タイプを取得（Catalog Items API）
// ============================================
function getProductTypeByAsin(accessToken, asin) {
  ...
  if (statusCode !== 200) {
    throw new Error("商品タイプ取得失敗 (ASIN: " + asin + "): HTTP " + statusCode);
  }
  ...
  try {
    ...
    throw new Error("商品タイプ解析失敗 (ASIN: " + asin + ")");
  }
  
  // productTypesから商品タイプを取得
  if (json.productTypes && json.productTypes.length > 0) {
    const productType = json.productTypes[0].productType;
    Logger.log("ASIN: " + asin + " → ProductType: " + productType);
    return productType;
  }
  
  // 取得できない場合はエラー
  throw new Error("商品タイプが見つかりません (ASIN: " + asin + ")");
}
...
function putListing(accessToken, sku, asin, price) {
  // ASINから商品タイプを取得
  const productType = getProductTypeByAsin(accessToken, asin);
...
function detectErrorType(message) {
  ...
  if (lowerMessage.indexOf("asin") !== -1 ||
      ...
      lowerMessage.indexOf("商品タイプ取得失敗") !== -1 ||
      lowerMessage.indexOf("商品タイプが見つかりません") !== -1 ||
      lowerMessage.indexOf("商品タイプ解析失敗") !== -1) {
    return "ASIN_ERROR";
  }
